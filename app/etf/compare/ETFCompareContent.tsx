  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9]">
      <div className="px-4 sm:px-8 py-6 sm:py-8 max-w-6xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900" style={{ fontFamily: 'Space Grotesk, system-ui, sans-serif' }}>
          ...
          <div className="flex items-center gap-2 flex-wrap mt-3">
            {symbols.map((t) => (
              <span key={t} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full glass-card-light text-sm font-mono font-black text-slate-800">
                {t}
                <button onClick={() => removeSymbol(t)} aria-label={`Remove ${t}`} className="text-slate-400 hover:text-slate-600 transition-colors">
                  <X size={12} />
                </button>
              </span>
            ))}
        ...
        {loading ? (
          ...
          <div className="glass-card-light rounded-xl p-8 text-center">
            <p className="text-sm text-slate-500">No ETF data found. Try a different ticker.</p>
          </div>
        ) : (
          <>
            {/* Metrics comparison table */}
            <section>
              <h2 className="text-base font-bold text-slate-900 mb-3" style={{ fontFamily: 'Space Grotesk, system-ui, sans-serif' }}>
                Metrics
              </h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {profiles.map((p) => (
                  <div key={p.ticker} className="glass-card-light rounded-xl p-4">
                    <p className="text-sm font-black font-mono text-slate-900 mb-3">{p.ticker}</p>
                    <div className="space-y-1.5">
                      {p.holdings.slice(0, 5).map((h) => (
                        <div key={h.symbol} className="flex items-center justify-between gap-2">
                          <span className={`font-mono font-bold text-xs ${sharedHoldings.has(h.symbol) ? 'text-blue-600' : 'text-slate-700'}`}>
                            {h.symbol}
                          </span>
                          <span className="text-xs text-slate-400 truncate flex-1 mx-2">{h.name}</span>
                    ...
                    {sharedHoldings.size > 0 && (
                      <p className="text-[10px] text-blue-500 font-semibold mt-2">Blue = shared across ETFs</p>
                    )}
                  </div>
                ))}
              </div>
