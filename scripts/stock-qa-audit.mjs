...
function auditFinancialConsistency(fin, stmts) {
  ...
  if (revenues.length >= 3) {
    ...
    if (calculatedCAGR != null && apiCAGR != null) {
      const diffBps = Math.abs(calculatedCAGR - apiCAGR) * 10000
      const label = isForeign ? 'WARN (FX may explain)' : diffBps > 200 ? 'WARN' : 'OK'
      if (diffBps > 500 && !isForeign) {
        issues.push(issue('WARN', 'CAGR_MISMATCH',
          `Historical revenue CAGR ${(calculatedCAGR * 100).toFixed(1)}% vs api.cagr ${(apiCAGR * 100).toFixed(1)}% — ${diffBps.toFixed(0)}bps gap`,
          { calculatedCAGR, apiCAGR, diffBps, label }
        ))
...
function auditValuationMethods(fin, stmts) {
  ...
  // CAGR consistency: api.cagr vs cagrAnalysis.blended vs cagrAnalysis.analystEstimate1y
  const cagrValues = {
    'api.cagr':                       fin.cagr,
    'cagrAnalysis.blended':           fin.cagrAnalysis?.blended,
    'cagrAnalysis.analystEstimate1y': fin.cagrAnalysis?.analystEstimate1y,
    'cagrAnalysis.historicalCagr3y':  fin.cagrAnalysis?.historicalCagr3y,
  }
