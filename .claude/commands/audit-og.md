...
After completing all phases, write every novel issue found to
`testing/findings/audit-log.md` using the standard format:
...
**Observed:** [exact file, line, value seen]
**Expected / correct behaviour:** [what it should be]
...
If any finding has `Run count: 2+` and `Status: confirmed`:
1. Insert the check into the relevant phase section of this file
2. Update `Status:` → `integrated`
3. Overwrite `.claude/commands/audit-og.md` with the updated content of this file

**0E. Sync to command**

After any write to `testing/og-design-audit-agent.md`, always overwrite
`.claude/commands/audit-og.md` with the identical content.
