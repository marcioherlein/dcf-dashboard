...
function sentimentColor(label: SentimentLabel): string {
  if (label === 'Risk-On')      return 'bg-emerald-100 text-emerald-800 border-emerald-200'
  if (label === 'Constructive') return 'bg-blue-100 text-blue-800 border-blue-200'
  if (label === 'Neutral')      return 'bg-slate-100 text-slate-700 border-slate-200'
  if (label === 'Cautious')     return 'bg-amber-100 text-amber-800 border-amber-200'
  return 'bg-red-100 text-red-800 border-red-200'
}

function vixColor(vix: number): string {
  if (vix < 15) return 'text-amber-600'
  if (vix < 20) return 'text-emerald-700'
    ...
    <div className="rounded-xl border border-slate-200 bg-white px-5 py-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="text-center">
          ...
          <p className="text-2xl font-extrabold font-mono tabular-nums text-slate-800">
            {tnxYield.toFixed(2)}%
          </p>
