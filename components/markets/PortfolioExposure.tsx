...
export default function PortfolioExposure({ portfolioExposure, modelAlerts }: Props) {
  ...
  if (totalSaved === 0) {
    return (
      <div className="bg-white rounded-xl border border-[#E5E5E5] shadow-sm px-5 py-6 flex flex-col items-center justify-center min-h-[140px] text-center">
        <p className="text-sm font-semibold text-[#111111]">No saved valuations yet</p>
        <p className="text-xs text-[#6B6B6B] mt-1 max-w-xs">Save a DCF valuation from any stock page to see model fragility alerts here.</p>
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E5E5E5] shadow-sm overflow-hidden">
      <div className="px-4 py-2.5 border-b border-[#E5E5E5] flex items-center justify-between">
        <span className="text-[11px] font-bold text-[#6B6B6B]">Saved Valuations</span>
        <span className="text-[11px] text-[#6B6B6B]">{totalSaved} model{totalSaved !== 1 ? 's' : ''}</span>
      </div>
      <div className="px-5 py-4 space-y-3">

        <div className="grid grid-cols-3 gap-2">
          <div className="rounded-xl border border-[#E5E5E5] bg-[#FCEAEA] px-3 py-3 flex flex-col">
            <p className="text-[20px] font-bold text-[#D83B3B] tabular-nums leading-none">{highCount}</p>
            <p className="text-[11px] text-[#D83B3B] font-semibold mt-1">High Risk</p>
          ...
          <div className="rounded-xl border border-[#E5E5E5] bg-[#FFF4DA] px-3 py-3 flex flex-col">
            <p className="text-[20px] font-bold text-[#B56A00] tabular-nums leading-none">{medCount}</p>
            <p className="text-[11px] text-[#B56A00] font-semibold mt-1">Medium</p>
          ...
          <div className="rounded-xl border border-[#E5E5E5] bg-[#E8F7EF] px-3 py-3 flex flex-col">
            <p className="text-[20px] font-bold text-[#11875D] tabular-nums leading-none">{healthCount}</p>
            <p className="text-[11px] text-[#11875D] font-semibold mt-1">Healthy</p>
        ...
        {uniqueTickers.length > 0 && (
          <div>
            <p className="text-[11px] font-bold text-[#6B6B6B] mb-1.5">Flagged Tickers</p>
            <div className="flex flex-wrap gap-1.5">
              {uniqueTickers.map(t => (
                <Link
                  key={t}
                  href={`/stock/${t}`}
                  className="text-[11px] font-bold bg-[#FCEAEA] border border-[#E5E5E5] text-[#D83B3B] px-2 py-0.5 rounded hover:bg-[#FCEAEA] transition-colors"
                >
                  {t}
                </Link>
              ))}
