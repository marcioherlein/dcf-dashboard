...
export default function PortfolioExposure({ portfolioExposure, modelAlerts }: Props) {
  ...
  if (totalSaved === 0) {
    return (
      <div className="rounded-xl border border-slate-200 bg-white px-5 py-4 flex flex-col items-center justify-center min-h-[140px] text-center">
        <p className="text-sm font-semibold text-slate-600">No saved valuations yet</p>
    ...
    <div className="rounded-xl border border-slate-200 bg-white px-5 py-4 space-y-3">
      ...
      <div className="rounded-lg bg-slate-50 border border-slate-100 px-3 py-2.5">
        ...
        <div className="flex items-center gap-4">
          <div>
            <p className="text-xl font-bold text-slate-800 font-mono">{modelAlerts.filter(a => a.severity === 'high').length}</p>
            <p className="text-[10px] text-red-600 font-semibold">High Risk</p>
          </div>
          <div>
            <p className="text-xl font-bold text-slate-800 font-mono">{modelAlerts.filter(a => a.severity === 'medium').length}</p>
            <p className="text-[10px] text-amber-600 font-semibold">Medium Risk</p>
          </div>
          <div>
            <p className="text-xl font-bold text-slate-800 font-mono">{totalSaved - uniqueTickers.length}</p>
