        ...
        <div>
          <div className="flex items-baseline gap-3 flex-wrap">
            ...
            {profile.priceChangePct != null && (
              <div className={cn('flex items-center gap-1 text-[14px] font-semibold', isUp ? 'text-[#11875D]' : 'text-[#D83B3B]')}>
                {isUp ? <TrendingUp size={14} /> : <TrendingDown size={14} />}
                <span>{fmtPct(profile.priceChangePct / 100)}</span>
              </div>
            )}
          </div>
          <p className="text-[13px] text-[#566174] mt-0.5">{profile.name}</p>
          {profile.fiftyTwoWeekHigh != null && profile.fiftyTwoWeekLow != null && (
            <p className="text-[12px] text-[#8A95A6] mt-1">
