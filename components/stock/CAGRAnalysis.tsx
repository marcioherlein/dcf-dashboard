...
const confColor: Record<string, string> = {
  ...
  Low:    'bg-[#FCEAEA] text-[#D83B3B] border-[#F0B8B8]',
}
  ...
  return (
    <div className="rounded-xl card p-6">
      ...
      {isNegativeFCF && (() => {
        ...
        return (
          <div className="mb-4 rounded-xl border border-[#F3D391] bg-[#FFF4DA] px-4 py-2.5 text-xs text-[#B56A00]">
            {ocfProxy ? (
        ...
        <table className="w-full min-w-[300px] text-sm">
          <thead>
            <tr className="border-b border-[#E3E1DA]">
              <th className="pb-2 text-left text-xs font-medium text-[#8A95A6]">Source</th>
              <th className="pb-2 text-right text-xs font-medium text-[#8A95A6]">Rate</th>
              <th className="pb-2 text-right text-xs font-medium text-[#8A95A6]">Note</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => (
              ...
              >
                ...
                <td className="py-2 text-right text-xs text-[#8A95A6]">{row.note}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="mb-4 rounded-xl border border-[#E3E1DA] bg-[#F4F3EF] px-4 py-3">
        ...
        {growthModel === 'three-stage' ? (
          <p className="mt-1.5 text-[11px] text-[#8A95A6] leading-relaxed">
            <span className="text-[#566174]">Years 1–5</span> at {fmtPct(ca.blended)}.{' '}
      ...
      {/* Growth drivers */}
      <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#8A95A6]">Assumptions &amp; Adjustments</p>
      <ul className="space-y-1.5">
        {ca.drivers.map((driver, i) => (
          <li key={i} className="flex items-start gap-2 text-xs text-[#566174]">
            <span className="mt-0.5 h-1.5 w-1.5 shrink-0 rounded-full bg-[#8A95A6]" />
            {driver}
          </li>
        ))}
      </ul>
