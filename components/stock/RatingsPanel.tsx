      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex gap-0.5">
          {[1,2,3,4,5].map((i) => (
            <div
              key={i}
              className={`h-1.5 w-1.5 rounded-full ${i <= dots ? 'bg-slate-700' : 'bg-slate-200'}`}
            />
          ))}
        </div>
      </div>
  ...
  return (
    <div className="rounded-xl card p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-sm font-headline font-semibold text-slate-900">Analysis Ratings</h2>
          <p className="mt-0.5 text-xs text-slate-400">Profitability · Liquidity · Growth · MOAT · Valuation</p>
        </div>
