      ...
      <div className="flex items-center gap-1.5 px-5 pt-3 pb-2 flex-wrap">
        ...
        {PERIODS.map(p => (
          <button key={p} onClick={() => setPeriod(p)}
            ...
            className={`rounded-lg px-2.5 py-2.5 text-[11px] font-[600] transition-colors min-h-[44px] ${period === p ? 'text-[#25310b]' : 'text-[#566174] border border-[#E5E5E5] bg-white hover:bg-[#F0F1F6] hover:border-[#CDD1C8]'}`}
        ...
        {ticker.toUpperCase() !== 'SPY' && (() => {
          ...
          return (
            <button
              ...
              aria-pressed={spyOn}
              className={`inline-flex items-center gap-1 rounded-lg px-2.5 py-2.5 text-[11px] font-[600] border transition-all min-h-[44px] ${spyOn ? 'bg-[#fff3e0] border-[#f97316] text-[#c2410c]' : 'border-[#E5E5E5] text-[#566174] hover:border-[#CDD1C8] hover:bg-[#F0F1F6]'}`}
            >
        ...
        {compareTickers.filter(t => t !== 'SPY').map((ct) => {
          ...
          return (
            <span key={ct} className="inline-flex items-center gap-1 rounded-lg px-2 py-2.5 text-[11px] font-[600] border min-h-[44px]"
        ...
        {compareTickers.length < 3 && (
          <form onSubmit={e => { e.preventDefault(); addCompareTicker(compareInput) }}>
            <input ref={compareInputRef} type="text" value={compareInput}
              ...
              className="inline-flex items-center rounded-lg px-2.5 py-2.5 text-[11px] font-[600] border border-dashed border-[#CDD1C8] bg-white focus:outline-none focus:border-[#5F790B] w-20 min-h-[44px] placeholder:text-[#8A95A6] text-[#06101F]"
            />
          </form>
        )}
        ...
        {isCompare && (
          <button onClick={() => setCompareMode(p => p === 'price' ? 'percent' : 'price')}
            className={`text-[11px] font-[600] px-2.5 py-2.5 rounded-lg border transition min-h-[44px] ${compareMode === 'percent' ? 'bg-olive-700 text-white border-olive-700' : 'border-[#E5E5E5] text-[#566174] hover:bg-[#F0F1F6]'}`}>
