...
interface Props { ticker: string; isDark?: boolean }

interface Bar { date: string; close: number }

export default function PriceChart({ ticker, isDark }: Props) {
  ...
  const tooltipStyle = isDark
    ? { background: '#1c1c1e', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, fontSize: 12, color: '#fff' }
        ...
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data}>
              ...
              <YAxis domain={[min, max]} tick={{ fontSize: 10, fill: tickFill }} tickLine={false} axisLine={false} tickFormatter={(v) => `$${v.toFixed(0)}`} width={50} />
              <Tooltip
