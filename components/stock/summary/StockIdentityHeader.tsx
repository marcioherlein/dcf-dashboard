...
const PriceChart = dynamic(() => import('@/components/stock/PriceChart'), {
  ssr: false,
  loading: () => (
    <div className="motion-safe:animate-pulse rounded-xl bg-[#F5F5F5]" style={{ minHeight: 340 }} />
  ),
})
...
function MetricRow({ label, value, last, sentiment }: { label: string; value: string; last?: boolean; sentiment?: Sentiment }) {
  const valueColor = sentiment === 'positive' ? 'text-[#11875D]' : sentiment === 'negative' ? 'text-[#D83B3B]' : 'text-[#111111]'
  return (
    <div className={`flex items-center justify-between py-2 ${last ? '' : 'border-b border-[#F0F0F0]'}`}>
      <span className="text-[11.5px] text-[#6B6B6B] leading-none">{label}</span>
      <span className={`text-[11.5px] font-[650] leading-none tabular-nums ${valueColor}`}>{value}</span>
    </div>
  )
}
...
export default function StockIdentityHeader({
  ...
  const roeStr      = roe != null && isFinite(roe) ? (roe * 100).toFixed(1) + '%' : '—'
  const roicStr     = roic != null && isFinite(roic) ? (roic * 100).toFixed(1) + '%' : '—'
  const divYieldStr = dividendYield != null && isFinite(dividendYield) && dividendYield > 0 ? (dividendYield * 100).toFixed(2) + '%' : '—'

  const tags = useMemo(() => {
    const r: string[] = []
    if (sector)   r.push(sector)
    if (industry) r.push(industry)
    if (dividendYield != null && dividendYield > 0) r.push('Dividend')
    return r
  ...
  const netSentiment: Sentiment       = netMargin    == null ? 'neutral' : netMargin > .15   ? 'positive' : netMargin  < 0   ? 'negative' : 'neutral'
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="bg-white border border-[#E5E5E5] rounded-xl p-5 flex flex-col sm:flex-row gap-5"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-3.5">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="text-[20px] sm:text-[22px] font-bold leading-tight text-[#111111] tracking-tight truncate">{companyName}</h1>
              <p className="mt-0.5 text-[13px] text-[#6B6B6B] truncate">
                <span className="font-semibold text-[#111111]">{ticker}</span>
                {companyType ? ` · ${companyType}` : ''}{country ? ` · ${country}` : ''}
              </p>
        ...
        <div className="hidden sm:flex flex-shrink-0 flex-col sm:basis-[34%] sm:max-w-[34%] sm:min-w-[200px]">
          ...
          {/* Valuation narrative — clickable rows, not competing price boxes */}
            ...
            <div className="mt-4 pt-3 border-t border-[#F0F0F0] flex flex-col gap-1">
              ...
              {fairValue != null && upside != null && (
                <button onClick={onViewValuation}
                  className="group flex items-center justify-between gap-2 text-left hover:bg-[#F5F5F5] rounded-lg px-2 py-2 -mx-2 transition-colors">
                  <div>
              ...
              {onViewConviction && (
                <button onClick={onViewConviction}
                  className="flex items-center gap-1.5 px-2 py-1.5 -mx-2 rounded-lg hover:bg-[#F5F5F5] transition-colors">
                  <svg className="w-3.5 h-3.5 text-[#5F790B] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.2}>
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-stretch">
            ...
            <div className="flex items-center gap-4 px-4 py-2 border-t border-[#F0F0F0]">
              {fairValue != null && (
                <span className="flex items-center gap-1.5 text-[11px] text-[#6B6B6B]">
                  <span className="w-3 h-0.5 rounded-full inline-block bg-[#5F790B]" />
                  Insic estimate
                </span>
              )}
              ...
              {userModelFairValue != null && (
                <span className="flex items-center gap-1.5 text-[11px] text-[#6B6B6B]">
                  <span className="w-3 h-0.5 rounded-full inline-block bg-[#B56A00]" />
                  Your model
                </span>
              )}
            </div>
        ...
        <div className="bg-white border border-[#E5E5E5] rounded-xl flex flex-col w-full sm:basis-[34%] sm:max-w-[34%] sm:min-w-[200px]"
          ...
          <div className="px-4 pt-4 pb-2 flex-1">
            ...
            <MetricRow label="P/E (TTM)"        value={peRatio != null ? peRatio.toFixed(2)+'×' : '—'}                   sentiment={peSentiment} />
            <MetricRow label="Revenue Growth"   value={revenueGrowth != null ? (revenueGrowth*100).toFixed(1)+'%' : '—'} sentiment={revGrowthSentiment} />
            <MetricRow label="EV/EBITDA"        value={evToEbitda != null ? evToEbitda.toFixed(2)+'×' : '—'}             sentiment={evSentiment} />
            <MetricRow label="ROE"              value={roeStr}                                                             sentiment={roeSentiment} />
            <MetricRow label="ROIC"             value={roicStr}                                                            sentiment={roicSentiment} />
            ...
            <MetricRow label="Gross Margin"     value={grossMargin != null ? (grossMargin*100).toFixed(1)+'%' : '—'}     sentiment={grossSentiment} />
            <MetricRow label="Net Margin"       value={netMargin != null ? (netMargin*100).toFixed(1)+'%' : '—'}         sentiment={netSentiment} />
            <MetricRow label="FCF Margin"       value={fcfMargin != null ? (fcfMargin*100).toFixed(1)+'%' : '—'}        sentiment={fcfSentiment} last />
          </div>
