...
const VERDICT_STYLE = {
  Undervalued:         { text: 'text-[#11875D]' },
  'Fairly Valued':     { text: 'text-[#2563EB]' },
  Overvalued:          { text: 'text-[#D83B3B]' },
  'Insufficient Data': { text: 'text-[#566174]' },
}
...
export default function SummaryCards({ output, currentPrice, changePct, currency, starRating }: Props) {
  ...
  const upColor = output.upsidePct != null ? (output.upsidePct >= 0 ? 'text-[#11875D]' : 'text-[#D83B3B]') : 'text-[#8A95A6]'
    ...
    <div className="bg-white rounded-[14px] border border-[#E6ECF5] shadow-[0_1px_2px_rgba(15,23,42,0.04)] overflow-hidden">
      <div className="grid grid-cols-2 md:grid-cols-5 gap-px bg-[#F4F3EF]">
        ...
        <div className="bg-white px-5 py-4 flex flex-col gap-1">
          ...
          <p className="text-[20px] font-[750] tabular-nums text-[#06101F] leading-none">
            {fmtPrice(currentPrice, currency)}
          </p>
          {changePct != null && (
            <p className={`text-[11px] font-[650] tabular-nums ${changePct >= 0 ? 'text-[#11875D]' : 'text-[#D83B3B]'}`}>
        ...
        {/* Blended Fair Value */}
        <div className="bg-white px-5 py-4 flex flex-col gap-1 border-t-2 border-[#2563EB]">
        ...
        <div className="bg-white px-5 py-4 flex flex-col gap-1">
          ...
          <p className="text-[11px] text-[#566174]">vs current price</p>
        </div>
            ...
            >
              <div className="flex items-center gap-0.5" aria-hidden="true">
                {Array.from({ length: 5 }, (_, i) => (
                  <svg
                    key={i}
                    className={`w-2.5 h-2.5 ${i < starRating.stars ? STAR_COLORS[starRating.stars] : 'text-[#CDD1C8]'}`}
              ...
              <span className="text-[10px] font-[650] text-[#566174]">{starRating.label}</span>
            </div>
          )}
          {!starRating && output.upsidePct != null && (
            <p className="text-[11px] text-[#566174]">{methodsComputed} of {output.methods.length} models</p>
          )}
        </div>

        {/* Market-Implied CAGR */}
        <div className="bg-white px-5 py-4 flex flex-col gap-1 col-span-2 md:col-span-1">
          ...
          {output.marketImpliedGrowth != null ? (
            <>
              ...
              <p className="text-[11px] text-[#566174]">5Y CAGR priced in by market</p>
            </>
          ) : (
            <p className="text-[13px] text-[#8A95A6]">—</p>
          )}
        </div>
