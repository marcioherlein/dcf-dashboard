...
function ScenarioCard({
  ...
  return (
    <div className={`rounded-xl border ${bgClass} ${borderClass} px-4 py-4 ${isBase ? 'ring-2 ring-blue-300 shadow-sm' : ''}`}>
      ...
      <p className={`${isBase ? 'text-3xl' : 'text-2xl'} font-bold tabular-nums text-slate-900 leading-none mb-2`}>
        {fv != null ? fmtPrice(fv, currency) : '—'}
      </p>
      <p className="text-[11px] text-slate-500 leading-snug mb-3">{rationale}</p>
  ...
  return (
    <div className="bg-white rounded-[20px] border border-[#E6ECF5] shadow-[0_1px_2px_rgba(15,23,42,0.04)] px-5 py-4">
      <div className="flex items-center justify-between mb-1">
        ...
        <span className="text-[11px] text-[#64748B]">All four methods re-run at each stress, same blend as Base</span>
      </div>
