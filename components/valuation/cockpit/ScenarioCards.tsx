...
function ScenarioCard({
  ...
  return (
    <div className={`rounded-xl border ${bgClass} ${borderClass} px-4 py-4 min-w-[220px] sm:min-w-0 snap-start flex-shrink-0 sm:flex-shrink sm:w-full ${isBase ? 'ring-2 ring-blue-300 shadow-sm' : ''}`}>
...
export default function ScenarioCards({ scenarios, currentPrice, currency }: Props) {
  ...
  return (
    <div className="bg-white rounded-[20px] border border-[#E6ECF5] shadow-[0_1px_2px_rgba(15,23,42,0.04)] px-5 py-4">
      ...
      <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory pb-1 -mx-1 px-1 sm:flex-col sm:overflow-visible sm:snap-none sm:pb-0 sm:mx-0 sm:px-0">
        <ScenarioCard
