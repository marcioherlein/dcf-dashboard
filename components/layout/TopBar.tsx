...
function CompanyLogo({ ticker }: { ticker: string }) {
  ...
  return (
    <Image
      ...
      width={24}
      height={24}
      className="rounded-full border border-[rgba(255,255,255,0.12)] shrink-0 object-cover"
    ...
    <header className="fixed top-0 left-0 lg:left-[240px] right-0 z-40 glass-toolbar" data-topbar>
      ...
      {stockNav && (
        <div className="sm:hidden flex items-center justify-between px-3 gap-2" style={{ height: '52px' }}>
          {mobileSearchOpen ? (
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              <button
                ...
                className="min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg text-[rgba(255,255,255,0.4)] hover:text-[rgba(255,255,255,0.6)] shrink-0"
            ...
            <>
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span className="font-bold text-[14px] text-white tracking-tight shrink-0">
                  {stockNav.ticker}
                </span>
                {stockNav.price != null && (
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button
                  ...
                  className="min-h-[44px] min-w-[44px] flex items-center justify-center rounded-lg text-[rgba(255,255,255,0.4)] hover:text-olive-700 hover:bg-olive-50 transition-colors"
          ...
          {mobileSearchOpen && open && results.length > 0 && (
            <div className="absolute left-0 right-0 z-50 bg-white border-b border-[rgba(255,255,255,0.12)] shadow-lg" style={{ top: 'calc(52px + env(safe-area-inset-top, 0px))' }}>
              {results.slice(0, 6).map(r => (
                ...
                >
                  ...
                  <span className="text-[12px] text-[rgba(255,255,255,0.6)] truncate flex-1">{r.longname ?? r.shortname}</span>
                  <span className="text-[10px] text-[rgba(255,255,255,0.4)] shrink-0">{r.exchDisp ?? r.exchange}</span>
                </button>
              ))}
            </div>
          )}
      ...
      >
        ...
        {stockNav ? (
          <>
            ...
            <div className="flex items-center min-w-0 gap-2 pl-1 lg:hidden">
              <CompanyLogo ticker={stockNav.ticker} />
              <span className="font-bold text-[13px] text-white tracking-tight shrink-0">
                {stockNav.ticker}
              </span>
              {stockNav.price != null && (
              ...
              <div className="relative w-full sm:w-auto sm:max-w-[480px]" ref={searchRef}>
                <div
                  ...
                  style={{
                    ...
                    borderColor: open ? 'rgba(95,121,11,0.7)' : 'rgba(255,255,255,0.15)',
                    boxShadow: open ? '0 0 0 3px rgba(95,121,11,0.18)' : 'none',
                  }}
                >
                  {loading ? (
                    <div className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-[rgba(255,255,255,0.12)] border-t-[#5F790B] shrink-0" />
                  ) : (
                    <Search size={14} className={searchError ? 'text-[#D83B3B] shrink-0' : 'text-[rgba(255,255,255,0.4)] shrink-0'} />
                  )}
                  <input
                    type="text"
                    role="combobox"
                    ...
                    className="flex-1 min-w-0 bg-transparent text-[16px] sm:text-[13px] text-white placeholder-[rgba(255,255,255,0.35)] focus:outline-none"
                    autoCorrect="off"
                    autoCapitalize="characters"
                    spellCheck={false}
                  />
                      ...
                      >
                        {results.map((r, idx) => (
                          <motion.button
                            ...
                            className={`flex w-full items-center justify-between gap-3 px-4 py-3 text-left border-b border-[rgba(255,255,255,0.12)] last:border-b-0 transition-colors ${
                          ...
                          >
                            <div className="min-w-0">
                              <div className="flex items-center gap-2 mb-0.5">
                                <span className="text-[14px] font-bold text-white font-mono">{r.symbol}</span>
                                {r.exchange && r.supported && (
                                  <span className="text-[10px] text-[rgba(255,255,255,0.6)] font-medium uppercase">{r.exchange}</span>
                                )}
                              </div>
                              <span className="text-[12px] text-[rgba(255,255,255,0.6)] truncate block">{r.longname ?? r.shortname}</span>
                            </div>
                            {r.supported ? (
                              r.quoteType && (
                              ...
                              <span className="shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-md bg-[#FFFFFF] text-[rgba(255,255,255,0.6)] border border-[rgba(255,255,255,0.12)] whitespace-nowrap">
        ...
        <div className="flex items-center gap-2 sm:gap-3 justify-end min-w-0">
          ...
          {stockNav && (
            <div className="hidden lg:flex items-center gap-2 pl-1 border-r border-[rgba(255,255,255,0.12)] pr-3 mr-1 shrink-0">
              <CompanyLogo ticker={stockNav.ticker} />
          ...
          {stockNav && (
            <button
              onClick={() => onSaveRef.current?.()}
              className="flex items-center gap-1.5 text-[12.5px] font-semibold text-[#7C9A19] px-3 py-1.5 rounded-lg border border-[#5F790B] bg-transparent hover:bg-[rgba(95,121,11,0.15)] transition-colors whitespace-nowrap"
            >
          ...
          {session ? (
            <div className="flex items-center gap-2 shrink-0">
              ...
              {isPro && (
                <button
                  ...
                  className="text-[12px] text-[rgba(255,255,255,0.6)] hover:text-white transition-colors whitespace-nowrap hidden sm:block"
                >
                  Billing
                </button>
              )}
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button
                onClick={() => signIn('google')}
                className="text-[12.5px] text-[rgba(255,255,255,0.6)] hover:text-white transition-colors whitespace-nowrap hidden md:block"
              >
                Sign in
              </button>
            </div>
