...
function Row({ label, value, bold, highlight }: { label: string; value: React.ReactNode; bold?: boolean; highlight?: boolean }) {
  return (
    <div className={`flex justify-between py-1.5 text-xs border-b border-white/8 last:border-0 ${highlight ? 'bg-blue-500/10 -mx-4 px-4' : ''}`}>
      <span className={bold ? 'font-semibold text-slate-100' : 'text-slate-400'}>{label}</span>
      <span className={`font-mono ${bold ? 'font-semibold text-slate-100' : 'text-slate-200'}`}>{value}</span>
    </div>
  )
}
  ...
  return (
    <div className="rounded-xl glass-card border-[rgba(59,130,246,0.15)] overflow-hidden">
      <div className="px-5 py-3 border-b border-white/10 bg-white/5">
        <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-400">Valuation Output</h3>
      </div>
        ...
        <div className={`px-4 py-4 ${isFinancialSector ? 'opacity-50' : ''}`}>
          ...
          {financialCurrencyNote ? (
            ...
            <div className="mt-3 flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-200">Fair Value</span>
              <span className="font-mono font-bold text-base text-slate-100">{`${curr}${fmtPrice(ufcfFairValue, '')}`}</span>
            </div>
          )}
          <div className="mt-1.5 flex items-center justify-between text-xs">
            <span className="text-slate-400">vs {curr}{currentPrice.toFixed(2)}</span>
      ...
      <div className="px-5 py-2.5 border-t border-white/8 bg-white/5 text-[11px] text-slate-400">
